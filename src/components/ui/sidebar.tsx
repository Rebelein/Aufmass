...
>(
  (
    ...
    const isMobile = forceMobile !== undefined ? forceMobile : _isMobile
    ...
    const resizeInfo = React.useRef({ initialX: 0, initialWidth: 0, side: 'left' });


    const handleMouseMove = React.useCallback((e: MouseEvent) => {
      if (!isResizingRef.current) return;
      
      const { initialX, initialWidth, side } = resizeInfo.current;
    ...
    const handleResizeMouseDown = React.useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        const sidebarEl = document.querySelector('[data-resizable-sidebar="true"]');
        if (!sidebarEl) return;
        ...
        resizeInfo.current = {
            initialX: e.clientX,
            initialWidth: sidebarEl.getBoundingClientRect().width,
            side: side,
        };
    ...
    const contextValue = React.useMemo<SidebarContext>(
      ...
      [state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar, width, isResizing, handleResizeMouseDown]
    )
...
>(({ className, ...props }, ref) => {
  const { handleResizeMouseDown, isResizing } = useSidebar();
  ...
  return (
    <div
      ...
      className={cn(
        "absolute inset-y-0 w-1.5 touch-none",
  ...
  (
        ...
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          ...
          >
            <SheetHeader className="sr-only">
              ...
              <SheetDescription>
                Wählen Sie eine Kategorie, um die zugehörigen Artikel anzuzeigen.
              </SheetDescription>
            </SheetHeader>
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            "duration-200 fixed inset-y-0 z-10 hidden h-svh w-[var(--sidebar-width)] transition-[left,right,width] ease-linear lg:flex",
            side === "left"
