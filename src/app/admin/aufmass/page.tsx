...
const AufmassAdminPage = () => {
  ...
  const refreshData = async () => {
    const [cats, supps, arts] = await Promise.all([
      getCategoriesList(),
      getSuppliersList(),
      getArticlesList()
    ]);
  ...
  const handleUpdateSupplierAdmin = async (id: string, name: string) => {
    await updateSupplier(id, { name });
    await refreshData();
  };

  const handleDeleteSupplierAdmin = async (id: string) => {
  ...
  const handleReorderArticlesInCategory = async (categoryId: string, reorderedArticlesForCategory: Article[]) => {
    ...
    await batchUpdateArticles(updates);
    await refreshData();
  };
  ...
  const renderCategories = (parentId: string | null = null, level = 0): JSX.Element[] => {
    return categories
      ...
      .flatMap((category, index, filteredArray) => {
    ...
    <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in duration-700">
      ...
      {managingCategoryDetails && (
        <ArticleManagementDialog
            isOpen={isArticleManagementDialogOpen}
            onClose={() => { setIsArticleManagementDialogOpen(false); setManagingCategoryDetails(null); }}
            categoryName={managingCategoryDetails.name}
            categoryId={managingCategoryDetails.id}
            articles={articlesAdmin.filter(art => art.categoryId === managingCategoryDetails.id).sort((a,b) => (a.order ?? 0) - (b.order ?? 0))}
            onAddNewArticle={handleAddNewArticleToCategory}
      ...
      <SupplierManagementDialog
        isOpen={isSupplierManagementDialogOpen}
        onClose={() => setIsSupplierManagementDialogOpen(false)}
        suppliers={suppliers}
        onAddSupplier={handleAddSupplierAdmin}
        onUpdateSupplier={handleUpdateSupplierAdmin}
        onDeleteSupplier={handleDeleteSupplierAdmin}
      />
    </div>
  );
};
